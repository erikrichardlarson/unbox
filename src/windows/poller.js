...
class Poller {
    ...
    rekordboxRT() {
        ...
        // Launch real-time version rb 6, need support for 6.5.1, watch for changes to txt file, parse file and return track details
        fs.watchFile('current_track.txt', (curr, prev) => {
            // parse current_track.txt
        })
    }
    ...
    virtualDJ() {
        ...
        if (process.platform == 'darwin') {
            var tracklistFile = process.env.HOME + '/Documents/VirtualDJ/History/tracklist.txt';
        }
        else {
            var tracklistFile = process.env.USERPROFILE + '\\Documents\\VirtualDJ\\History\\tracklist.txt';
        }
        var tracks = fs.readFileSync(tracklistFile).toString().split('\n');        tracks.pop();
    ...
    rekordboxRealtime() {
        ...
        if (this.rekordboxPoller) {
            return
        }

        this.rekordboxPoller = exec(path.join(__dirname, 'rekordbox_realtime\\Launcher.exe'), function (err, data) { });
        
        var _this = this;

        fs.watchFile(path.join(__dirname, 'rekordbox_realtime\\OutputFiles\\current_tracks.txt'), (curr, prev) => {
            var tracks = fs.readFileSync(path.join(__dirname, 'rekordbox_realtime\\OutputFiles\\current_tracks.txt')).toString().split("\r\n");
            var history = [];
            ...
            for (var i = 0; i <= tracks.length - 1; i++) {
                ...
                if (i == 0) {
                    _this.currentTrackDetails['artist'] = trackData[0].trim();
                    _this.currentTrackDetails['track'] = trackData[1].trim();
                    _this.currentTrackDetails['label'] = trackData[2].trim();
                    _this.currentTrackDetails['remix'] = trackData[3].trim();
                }
            }
    ...
    denon() {
        ...
        denon().then(() => {
            ...
            fs.watchFile(path.join(__dirname, denonOutputPath), (curr, prev) => {

                let deckState = JSON.parse(fs.readFileSync(path.join(__dirname, denonOutputPath), 'utf8'));
    ...
    mixxx() {
        ...
        db.serialize(function () {
            db.each(`select
            li.artist, li.title, pt.pl_datetime_added
